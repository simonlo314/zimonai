...
The knowledge hub's "Start here" article is the current editorial lead, not a permanent beginner article. Exactly one published article must carry `featured: true` at a time; array order must not decide which article is featured. Choose the article with the strongest current combination of reader interest, direct relevance to supplier or product verification, evidence quality, freshness and a clear visual entry point. A timely current-affairs article may outrank an evergreen reference when it meets those standards, but the newest article does not win automatically.

Review the selection whenever a new article is published. The featured specification must carry `featuredReviewedThrough` equal to the newest published article date, even when the existing selection remains the best choice. This makes a missed editorial review fail the taxonomy test instead of silently leaving an old article in place. When the selection changes, move both `featured: true` and `featuredReviewedThrough` to the new article in the same change.
