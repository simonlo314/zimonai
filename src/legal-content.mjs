      ...
      sections: [
        ...
        },
        {
          ...
          paragraphs: [
            ...
            'A portal account stores the verified email address, preferred language, internal account reference, case references, submitted case fields and status history needed to operate the workspace. Google passwords and Google access tokens are not stored by ZimonAI.'
          ]
        },
        {
  ...
  'zh-tw': {
    ...
    privacy: {
      ...
      sections: [
        ...
        { title: '案件文件與供應商資料', paragraphs: ['供應商查核可能需要公司名稱、企業紀錄、報價單、產品型號、證書編號、圖片、往來信件，以及這次查核要支援的買方決策。部分文件可能包含姓名或商務聯絡方式。', 'ZimonAI 只會用這些資料評估需求、確認範圍、執行查核、說明證據、交付報告、回答後續問題，以及保存可追溯的商業紀錄。客戶需確保自己有權提供相關文件與資訊。', '客戶中心會保存已驗證的 Email、偏好語言、內部帳戶編號、案件編號、送出的案件欄位與狀態紀錄。ZimonAI 不會保存你的 Google 密碼或 Google 存取權杖。'] },
  ...
  'zh-cn': {
    ...
    privacy: {
      ...
      sections: [
        ...
        { title: '案件文件与供应商资料', paragraphs: ['供应商核查可能需要企业名称、企业记录、报价单、产品型号、证书编号、图片、往来邮件，以及本次核查要支持的买方决策。部分文件可能包含姓名或商务联系方式。', 'ZimonAI 只会使用这些资料评估需求、确认范围、执行核查、说明证据、交付报告、回答后续问题，以及保存可追溯的商业记录。客户需要确保自己有权提供相关文件与信息。', '客户中心会保存已验证的邮箱、偏好语言、内部账户编号、项目编号、提交的项目字段与状态记录。ZimonAI 不会保存你的 Google 密码或 Google 访问令牌。'] },
